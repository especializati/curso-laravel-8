...
<div class="grid">
    <a href="{{ route('posts.create') }}" class="my-4 uppercase px-8 py-2 rounded bg-green-600 text-blue-50 max-w-max shadow-sm hover:shadow-lg"><i class="fas fa-plus-circle"></i></i>Criar Novo Post</a>
</div>
@if (session('message'))
...
<form action="{{ route('posts.search') }}" method="post" class="bg-white">
    ...
    <div class="max-w-sm my-4 p-1 pr-0 flex items-center">
        ...
        <button type="submit" class="uppercase p-3 rounded bg-blue-500 text-blue-50 max-w-max shadow-sm hover:shadow-lg"><i class="fas fa-search"></i>Filtrar</button>
    </div>
</form>
        ...
        @foreach ($posts as $post)
            <tr>
                ...
                <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5 text-right">
                    <a href="{{ route('posts.show', $post->id) }}" class="px-5 py-2 border-blue-500 border text-blue-500 rounded transition duration-300 hover:bg-blue-700 hover:text-white focus:outline-none"><i class="fas fa-eye"></i>Ver</a>
                    <a href="{{ route('posts.edit', $post->id) }}" class="px-5 py-2 border-green-500 border text-green-500 rounded transition duration-300 hover:bg-green-700 hover:text-white focus:outline-none"><i class="fas fa-edit"></i></i>Editar</a>
                </td>
            </tr>
        @endforeach
    </tbody>
